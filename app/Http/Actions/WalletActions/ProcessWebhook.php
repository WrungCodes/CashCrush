...
class ProcessWebhook
{
    ...
    public function processCredit()
    {
        ...
        $quantity = $this->getCoinQuantity($transaction);

        $user = User::where(['id' => $transaction->user_id])->first();

        $currentBalance = $this->CreditCoin($user, $quantity);
    ...
    private function getCoinQuantity(Transaction $transaction)
    {
        if ($transaction->transaction_type == 'coin') {
            return CoinDeal::where(['uid' => $transaction->transaction_type_uid])->first()->quantity;
        }
    }
}
